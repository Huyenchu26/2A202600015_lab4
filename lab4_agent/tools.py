...
def search_flights(origin: str, destination: str) -> str:
    ...
    for flight in flights:
        result += f"- {flight['airline']}: {flight['departure']} - {flight['arrival']} ({flight['price']:,}đ)\n"
    return result
@tool
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    result = f"Các khách sạn tại {city} với giá dưới {max_price_per_night:,}đ/đêm:\n"
    for hotel in filtered_hotels:
        result += f"- {hotel['name']}: {hotel['stars']} sao, {hotel['price_per_night']:,}đ/đêm, {hotel['area']}, rating {hotel['rating']}\n"
    return result
@tool
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    try:
        ...
        for item in expenses.split(','):
            ...
            expenses_dict[key] = int(value)
        total_expenses = sum(expenses_dict.values())
        remaining_budget = total_budget - total_expenses
        result = f"Bảng chi phí:\n"
        for key, value in expenses_dict.items():
            result += f"- {key}: {value:,}đ\n"
        result += f"---\n"
        result += f"Tổng chi: {total_expenses:,}đ\n"
        result += f"Ngân sách: {total_budget:,}đ\n"
        result += f"Còn lại: {remaining_budget:,}đ\n"
        if remaining_budget < 0:
            result += f"Vượt ngân sách {abs(remaining_budget):,}đ! Cần điều chỉnh.\n"
        return result
    except Exception as e:
        return f"Lỗi: {e}. Vui lòng kiểm tra lại định dạng expenses."